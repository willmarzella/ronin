...
def _tune_system(variant: str, config: Optional[Dict[str, Any]] = None) -> str:
    ...
    return (
        ...
        "a role's bullets. ORDER within each role: social-verification bullets "
        "first (sign-off won, audit passed, number defended, workflow "
        "retired, regulator satisfied), then consequence bullets (named "
        "business function + what changed for them), then judgment-flavored "
        "build bullets last. METRICS: denominate in hours, dollars, risk, or "
