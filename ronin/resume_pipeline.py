...
def _tune_system(variant: str, config: Optional[Dict[str, Any]] = None) -> str:
    ...
    return (
        ...
        "Delivered. (2) Tools are the uniform, not the pitch: a bullet may "
        "close with its stack in parentheses as evidence, but a tool name "
        "is never the subject or opener of a bullet, and the dedicated "
        "tooling capability line carries the keyword load. Call "
        "architecture patterns by name where the facts support them "
        "(medallion Bronze/Silver/Gold, CDC, SCD Type 2, star schema, "
        "infrastructure-as-code). (3) BULLET REGISTER: every bullet is a "
        "verb-first past-tense fragment that states the consequence and "
        "carries the artifact and stack as the means, e.g. 'Removed a "
        "10-hour/week manual reconciliation by building the refund pipeline "
        "end to end (S3, SQS, Lambda, Snowflake)'. Flat and conservative — "
        "the reader is an Australian enterprise hiring manager skimming 40 "
        "resumes: no em dashes inside bullets (parentheses for stack, at "
        "most one semicolon), no noun-phrase openers, no wit or metaphor, "
        "no editorial meta-commentary ('X, not Y, was the deliverable' is "
        "banned). A boring sentence reporting a rare fact beats a vivid "
        "sentence reporting a common one. Vary sentence construction across "
        "a role's bullets. ORDER within each role: specification-and-"
        "verification bullets first (SME checks codified as tests, sign-off "
        "won, audit passed, grain or invariant checks, number defended, "
        "workflow retired, regulator satisfied), then consequence bullets "
        "(named business function + what changed for them), then "
        "implementation bullets last — implementation is the layer being "
        "automated and never leads. METRICS: denominate in hours, dollars, risk, or "
        "people — never in artifacts. PR counts, line counts, and file "
        "counts are banned; artifact counts (models, reports) may appear "
        "only as evidence-scale inside a consequence bullet, never as the "
        "lead. Never write 'followed existing patterns' or similar "
        "compliance phrasing. No invented outcomes: every claim "
        "specific-and-falsifiable, or stay artifact-framed — vague 'enabled "
        "better decisions' is worse than tech-first. "
        ...
        "discovery-to-production ownership). (5) The summary claims the "
        "domain (regulated, physical-asset data) and the judgment layer "
        "(specifying what must be true, verifying it stayed true) rather "
        "than tools. Shape it as: "
        ...
        "then 'Expert in ...' with the highest-signal keywords. At most one "
        "voice-line per summary is permitted — the sign-off-as-done sentence "
        "('delivery counts as done when the business signs off on the "
        "numbers, not when the pipeline deploys'), adapted to the variant's "
        "focus; every other summary sentence stays flat. (6) Write a "
