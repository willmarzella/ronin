        ...
        try:
            ...
            for record in job_records:
                ...
                try:
                    ...
                    self.logger.error(f"Error getting recruiter details for ID {recruiter_id}: {str(e)}")
                    continue
    ...
    def generate_outreach_content(self, job: Dict) -> Dict:
        ...
        try:
            ...
            if recruiter.get("phone"):
                text_content = self.outreach_service.generate_text_content(recruiter, job)
            ...
            self.logger.error(f"Error generating outreach content for job {job.get('title', 'Unknown')}: {str(e)}")
            return {
                "email": {"subject": "Error generating content", "body": "Please generate manually"},
                "text": None,
                "has_phone": False,
                "has_email": False
            }
    ...
    def generate_outreach_markdown(self, jobs_with_outreach: List[Dict]) -> str:
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"# Recruiter Outreach - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                f.write(f"Generated outreach content for {len(jobs_with_outreach)} jobs with recruiters.\n\n")
                f.write("---\n\n")
                ...
                # Summary section
                f.write("## 📊 Summary\n\n")
                email_count = sum(1 for job in jobs_with_outreach if job["outreach"]["has_email"])
                text_count = sum(1 for job in jobs_with_outreach if job["outreach"]["has_phone"])
    ...
    def process_jobs_for_outreach(self, status: str = "DISCOVERED") -> str:
        ...
        try:
            ...
            for job in jobs_with_recruiters:
                self.logger.info(f"Generating outreach for: {job['title']} (Recruiter: {job['recruiter']['name']})")
            ...
            self.logger.info(f"Outreach generation complete. Generated content for {len(jobs_with_outreach)} jobs")
